...
class Render():
    ...
    def __init__(self, input_dir, input_name):
        ...
        rollout_cases = [
            ["ground_truth_rollout", "Reality"], ["predicted_rollout", "Paddle"]]
        ...
        if self.dims == 2:
            def animate(i):
                ...
                for j, datacase in enumerate(trajectory_datacases):
                    ...
                    axes[j].xaxis.set_major_locator(plt.NullLocator())
                    axes[j].yaxis.set_major_locator(plt.NullLocator())
...
#--------------------------------------------------
        ...
        #
        #         fig.clear()
        #         for j, datacase in enumerate(self.rollout_cases):
        #             # select ax to plot at set boundary
        #             axes[j] = fig.add_subplot(1, 2, j + 1, autoscale_on=False)
        #             axes[j].set_aspect(1.)
        #
        #             # axes[j].set_xlim([-1, 2])
        #             # axes[j].set_ylim([-1, 2])
        #             axes[j].set_xlim([float(self.boundaries[0][0]), float(self.boundaries[0][1])])
        #             axes[j].set_ylim([float(self.boundaries[1][0]), float(self.boundaries[1][1])])
        #
        #             # Disable grid lines
        #             axes[j].xaxis.set_major_locator(plt.NullLocator())
        #             axes[j].yaxis.set_major_locator(plt.NullLocator())
        #
        #             for mask, color in self.color_mask():
        #                 axes[j].scatter(self.trajectory[datacase[0]][i][mask, 0],
        #                                 self.trajectory[datacase[0]][i][mask, 1], s=1, color=color)
        #             axes[j].grid(True, which='both')
        #             axes[j].set_title(datacase[1])
# --------------------------------------------------
            ...
            def animate(i):
                ...
                for j, datacase in enumerate(trajectory_datacases):
                    # if j==98:
                        ...
                        for mask, color in color_mask:
                            axes[j].scatter(self.trajectory[datacase][i][mask, 2],
...
#-----------------------------------------------------------------------------------
                ...
                #     axes[j] = fig.add_subplot(1, 2, j + 1, projection='3d', autoscale_on=False)
                #
                #     # Adjust the limits to control the height
                #     axes[j].set_xlim([float(xboundary[0]), float(xboundary[1]-0.1)])
                #     axes[j].set_ylim([float(yboundary[0]), float(yboundary[1])-0.1])
                #     axes[j].set_zlim([float(zboundary[0]), float(zboundary[1]+0.1)])
                #
                #     # Hide the coordinate information
                #     axes[j].set_xticks([])
                #     axes[j].set_yticks([])
                #     axes[j].set_zticks([])
                #     axes[j].axis('off')
        ...
        # ani = animation.FuncAnimation(
        #     fig, animate, frames=[200], interval=10)
