...
def get_random_walk_noise_for_position_sequence(
  ...
  """
    ...
    velocity_sequence_noise = paddle.randn(shape=list(velocity_sequence.shape)
        ) * (noise_std_last_step / num_velocities ** 0.5)
    velocity_sequence_noise = paddle.cumsum(x=velocity_sequence_noise, axis=1)
    # Integrate the noise in the velocity to the positions, assuming
    # an Euler intergrator and a dt = 1, and adding no noise to the very first
    # position (since that will only be used to calculate the first position
    # change).
    position_sequence_noise = paddle.concat(x=[paddle.zeros_like(x=
        velocity_sequence_noise[:, 0:1]), paddle.cumsum(x=
        velocity_sequence_noise, axis=1)], axis=1)
